...
const IdentificationView: React.FC<{ onIdentify: (cedula: string) => void; onScanClick: () => void; errorMessage: string; }> = ({ onIdentify, onScanClick, errorMessage }) => {
    const [cedula, setCedula] = useState('');
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (cedula.trim()) onIdentify(cedula.trim());
    };
    return (
         <div className="max-w-md mx-auto bg-white p-8 mt-10 rounded-xl shadow-lg border border-gray-200">
            <div className="flex items-center gap-4 mb-6">
                <div className="bg-red-100 p-4 rounded-full"><IdCardIcon className="w-8 h-8 text-red-600" /></div>
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">Paso 1: Identificar Pasajero</h2>
                    <p className="text-gray-500">Ingrese la cédula o escanee el código QR.</p>
                </div>
            </div>
             <div className="mb-6">
                <button onClick={onScanClick} type="button" className="w-full bg-gray-800 hover:bg-black text-white font-bold py-4 px-6 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-center gap-3 text-lg transform hover:-translate-y-0.5">
                    <QRIcon className="w-7 h-7" /> Escanear Código QR
                </button>
            </div>

            <div className="flex items-center text-center my-6">
                <div className="flex-grow border-t border-gray-300"></div>
                <span className="flex-shrink mx-4 text-gray-500 font-semibold">O</span>
                <div className="flex-grow border-t border-gray-300"></div>
            </div>

            <form onSubmit={handleSubmit}>
                <FormInput label="Nro. de Cédula" name="cedula" value={cedula} onChange={(e) => setCedula(e.target.value)} placeholder="Ej: 12345678" />
                 {errorMessage && (
                ...
                <div className="mt-6 flex justify-end">
                     <button type="submit" disabled={!cedula.trim()} className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 disabled:bg-red-300 disabled:cursor-not-allowed transform hover:-translate-y-0.5">
                        Buscar Pasajero
                    </button>
                </div>
            </form>
        ...
        {reports.length === 0 ? (
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={onStartNew} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-5 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 flex items-center gap-2">
...
const DriverView: React.FC<DriverViewProps> = ({ driver }) => {
    ...
    const handleScanSuccess = (scannedData: string) => {
        ...
        // Intentar parsear como JSON (nuevo formato de QR)
        try {
            const passengerData = JSON.parse(scannedData);
            if (passengerData.cedula) {
                handleIdentify(passengerData.cedula);
                return;
            }
        } catch (error) {
            // Si no es JSON, tratar como cédula directa (formato anterior)
            handleIdentify(scannedData);
            return;
        }
        
        // Si llegamos aquí, el formato no es reconocido
        setErrorMessage('Formato de QR no reconocido. Intente escanear nuevamente.');
    };
    ...
    const handleAddPassenger = async (scannedData: string) => {
        if (!activeReport) return;
        
        let cedula = scannedData;
        ...
        if (activeReport.pasajeros.some(p => p.passenger.nroCedula === cedula)) {
             setErrorMessage(`El pasajero con cédula "${cedula}" ya fue registrado.`);
             return;
        }
        const passenger = await mockApi.findPassengerByCedula(cedula);
        if (passenger) {
    ...
    const handleFinalizeTrip = async () => {
        if (activeReport) {
            setIsSending(true);
            try {
                const submittedReport = await mockApi.submitReport(activeReport);
    ...
    const renderContent = () => {
        switch (view) {
            ...
            case 'identify': return <IdentificationView onIdentify={handleIdentify} onScanClick={() => setIsQRScannerOpen(true)} errorMessage={errorMessage} />;
            case 'setup': return initialPassenger ? <SetupView passenger={initialPassenger} driver={driver} onStartTrip={handleStartTrip} /> : null;
            case 'active': return activeReport ? <ActiveReport report={activeReport} onAddPassenger={openManualScanner} onFinalize={handleFinalizeTrip} isSending={isSending} /> : null;
    ...
    return (
        <>
            ...
            <Toast 
                ...
                type={toastInfo.type}
            />
            {isScannerOpen && <ScannerModal onClose={() => setIsScannerOpen(false)} onScan={handleAddPassenger} errorMessage={errorMessage} />}
