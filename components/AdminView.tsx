...
import { DownloadIcon, SpinnerIcon, ErrorIcon, LogIcon, UsersIcon, TruckIcon, PlusIcon, TrashIcon, PencilIcon, QRIcon } from './Icons';
...
const QRCodeModal: React.FC<{ passenger: Passenger; onClose: () => void }> = ({ passenger, onClose }) => {
    ...
    useEffect(() => {
        if (qrRef.current && passenger.nroCedula) {
            // Safety check to ensure the library is loaded
            if (typeof window.QRCode === 'undefined') {
                console.error("QRCode library is not loaded yet.");
                setQrError(true);
                return;
            }
            window.QRCode.toCanvas(qrRef.current, passenger.nroCedula, { width: 256, margin: 2 }, (error: any) => {
                if (error) {
            ...
            <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-sm m-4 transform transition-all text-center" onClick={(e) => e.stopPropagation()}>
                <div ref={modalContentRef}>
                    ...
                    <div className="flex justify-center my-4 h-[256px] w-[256px] mx-auto">
                        {qrError 
                            ? <div className="text-red-500 bg-red-100 flex items-center justify-center w-full h-full rounded-lg p-4">Error al generar el código QR. La librería no se cargó.</div>
                            : <canvas ref={qrRef} />
                        }
                    </div>
    ...
    const showToast = (message: string, type: 'success' | 'error' = 'success') => {
     ...
     const handleDownloadReport = (reportToDownload: Report) => {
        ...
        if (typeof window.jspdf === 'undefined') {
            showToast('Error: La librería PDF no se ha cargado.', 'error');
            return;
        }
        ...
        const pdfFileName = `Reporte_${conductor.replace(/ /g, '_')}_${adjustedDate.toISOString().split('T')[0]}.pdf`;
        ...
        try {
            ...
            doc.setFontSize(11);
            doc.setFont('helvetica', 'normal');
            doc.text(adjustedDate.getDate().toString(), 21, dateY + 7, { align: 'center' });
            doc.text((adjustedDate.getMonth() + 1).toString(), 31, dateY + 7, { align: 'center' });
            doc.text(adjustedDate.getFullYear().toString().slice(-2), 41, dateY + 7, { align: 'center' });
